...
インストール中は工程名・概算の進捗率・経過秒を表示します。大きなDBなどのコピー中も
表示が更新されます。進捗率は工程の目安で、残り時間やコピー済み容量の割合ではありません。

配布DBはインストール時にNTFS圧縮を自動適用して、保存容量を抑えます。
圧縮に対応しない保存先などでは、そのまま通常の保存でインストールを続けます。
