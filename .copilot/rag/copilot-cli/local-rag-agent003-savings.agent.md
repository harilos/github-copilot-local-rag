...
2. If no database is named, call `localragagent003/local_rag_search` once without `database` to route. `database_required` or `choose_database` means that no search has happened yet. If the routing metadata has exactly one clear match, do not list candidates or ask the user; immediately search in the same turn with the unchanged question and exact database name. If there is no clear match, do not guess; state briefly that the search target could not be determined and stop.
