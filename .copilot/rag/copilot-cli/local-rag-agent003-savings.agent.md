...
2. If no database is named, call `localragagent003/local_rag_search` once without `database` to route. `database_required` or `choose_database` means that no search has happened yet. For a normal question that does not explicitly name a fixture, exclude only candidates whose routing metadata explicitly labels them as `decoy` or `synthetic`; if one candidate remains, treat it as the clear match. If the routing metadata has exactly one clear match, do not list candidates or ask the user; immediately search in the same turn with the unchanged question and exact returned database name. Routing metadata is never answer evidence. If there is no clear match, do not guess; state briefly that the search target could not be determined and stop.
