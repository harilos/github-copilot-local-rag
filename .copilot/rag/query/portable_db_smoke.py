    ...
    try:
        for name in args.databases:
            payload = run_search_payload(
                db_name=name,
                question="portable install health check",
                top_k=1,
                use_dense=False,
            ...
            if (
                payload.get("schema") != "local-rag.search.v1"
                or payload.get("selected_db") != name
                or payload.get("status") not in {"ok", "partial", "no_hit"}
            ):
