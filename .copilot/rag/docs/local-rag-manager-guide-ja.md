...
全DBの処理後は、最後に全DBのサマリーを表示します。成功・警告付き完了・
一部反映・失敗・スキップのSource数、DB別の件数、問題のあるDBとSourceの一覧を
確認できます。DB自体の処理失敗は、Sourceの失敗件数と分けて表示します。

本文を抽出できなかった文書（ZeroText）は「本文なし」の警告付き完了とし、
検索対象には含めません。読取りや抽出に失敗した文書が一部あっても、処理できた
文書は反映します。失敗した文書は同じSourceの更新・再開で再試行できます。

古い取得物や派生データを通常経路から外して全件取り直すには、メインメニューの
`7`を使います。実行前に、対象DB、Source数、保持する設定、切り離すデータ、
自動再取得できない例外をまとめて表示し、確認は一度だけ行います。

DB・Source設定、取得条件、認証、端末設定、Source Link、利用者が書いた説明、
外部の元資料は残ります。取得済みwork、clean data、catalog、vector index、
取込・再開状態、進捗、結果cacheは新しい世代から切り離されます。古いデータが
ディスク上の退避領域へ残っていても、検索、詳細取得、再開、再構築、次回更新、
配布、DBコピーからは参照されません。

この操作だけではnetwork取得、文書変換、embedding、再構築を開始しません。
取り直しを始めるときは続けて`3`を選びます。取得期間や除外条件は変えず、
現在登録されている範囲だけを再取得します。反映済み文書と検索DBの整合性が
確認できた時点から、その文書を検索できます。未完了のSourceが残っている場合は
検索結果に注意を表示し、全件完了の記録やDB配布・コピーとは区別します。
途中停止時も同じ`7`を再実行すれば続きから安全に収束します。

`Other`の一回取り込みなどworkが唯一の原本になり得るSourceは、通常経路外の
保護領域へ封印して残し、DBを`attention_required`として表示します。黙って
成功にはせず、そのSourceは再登録後の更新が必要です。他のSourceの文書が
反映済みで整合性を確認できれば、その文書の検索は利用できます。
登録Sourceが1件もないDBも`attention_required`となり、Sourceを登録してから
更新するよう案内します。例外のあるDBを全件成功として扱うことはありません。

自動運用では対話画面と同じcoreを次のように呼べます。`--plan-only`は変更せず
対象だけをJSON表示します。実行には明示的な`--yes`が必要です。

```powershell
& "$env:USERPROFILE\.copilot\rag\query\.venv\Scripts\python.exe" `
  -B "$env:USERPROFILE\.copilot\rag\manage.py" `
  reset-data --all --plan-only

& "$env:USERPROFILE\.copilot\rag\query\.venv\Scripts\python.exe" `
  -B "$env:USERPROFILE\.copilot\rag\manage.py" `
  reset-data --db example-rag --yes
```
